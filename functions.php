...
function axis_send_notification($post_ID, $post, $update) {

    if ($update) {return;};

    $target_post = array('application', 'contact');

    if (!in_array($post->post_type, $target_post)) {return;}

    $to = 'info@axismedical.gr';
    $form_name = ($post->post_type === 'contact') ? 'Contact' : 'Careers';

    $subject = 'New Message! Please check it out!';
    $message = 'Hello!, \n\n';
    $message .= 'You have received a new submission from the form $form_name';
    $message .= 'You can view the details in your dashboard here:\n';
    $message .= admin_url('post.php?=post=' . $post_ID . '&action=edit') . '\n\n';

    wp_mail($to, $subject, $message);

}
add_action('wp_insert_post', 'axis_send_notification', 10, 3);
/////////////////////////////////////////////////////////////////////////
...
function demo_request_submission() {
    ...
    $product = isset($_POST['product_name']) ? sanitize_text_field($_POST['product_name']) : 'General Product';
